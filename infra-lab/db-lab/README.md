...
이 트랙은 **관계형(PostgreSQL)** 스키마 운영을 다룬다.
같은 "무엇을 물어볼 수 있게 설계하는가" 문제를 DynamoDB 쪽에서 본 노트가 따로 있다.

- [DynamoDB 단일 vs 멀티 테이블 · 키 설계](../aws-lab/DynamoDB-단일-vs-멀티-테이블.md) —
  PK가 가능한 질의를 결정한다 · 정렬키 충돌 · `ScanIndexForward` 기본값
  → [`../aws-lab/`](../aws-lab/README.md)
