...
> 당시 로그는 남기지 않았지만, **원인은 확인됐다 — GitHub Secret 미등록.**
> 그 값이 어떤 경로로 컨테이너까지 흘러가 기동을 막았는지는 코드로 복원했다.
...
**GitHub Secret `SECRET_ARN`을 등록하지 않은 상태였다.**
`terraform apply` 출력에서 ARN을 복사해 저장소 Secrets에 넣는 수동 단계를 빠뜨린 것이다.

## 원인

**필수 설정(Secrets Manager ARN)을 등록하지 않았다.**

그런데 이 실수 자체보다 **그것이 드러난 방식**이 문제였다.

```
GitHub Secret 없음
  → ${{ secrets.SECRET_ARN }} 이 빈 문자열로 치환 (오류 아님)
    → 컨테이너 환경변수 SECRET_ARN=""       (오류 아님)
      → get_secret_value(SecretId="")       ← 여기서 처음 터짐
        → import 중 예외 → 컨테이너 즉시 종료
          → 헬스체크 30초 무응답, 로그도 없음  ← 여기서 처음 보임
```

**설정을 빠뜨린 지점과 그것이 드러난 지점 사이에 다섯 단계가 있었다.**
접속 정보가 예측 불가능한 값(ARN)이라 사람이 옮겨 적는 단계에 의존한 것이 근본 원인이다.
